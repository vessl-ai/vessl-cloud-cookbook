...
# If AQR_FULL_WEIGHT_DIR is set, the "adapter" slot becomes a full-weight CPT
# merged checkpoint loaded via AutoModelForCausalLM (HF format dir from axolotl
# merge-sharded-fsdp-weights). All downstream output keys stay the same so the
# cookbook + benchmarks.md format is unchanged across LoRA and full-weight runs.
_FULL_WEIGHT_DIR_ENV = os.environ.get("AQR_FULL_WEIGHT_DIR")
FULL_WEIGHT_DIR = Path(_FULL_WEIGHT_DIR_ENV) if _FULL_WEIGHT_DIR_ENV else None
...
# Robustness add-ons (ADDITIVE — original r2_off/r2_on/premium keys unchanged).
EMBARGO_DAYS = int(os.environ.get("AQR_EMBARGO_DAYS", "5"))   # sampled trading-day gap dropped before the chronological test boundary (JPX Target ~T+2, so >=2 is safe)
WF_N_SPLITS = int(os.environ.get("AQR_WF_N_SPLITS", "5"))     # expanding walk-forward folds for the multi-period robustness R^2
EMBARGO_IN_BOOTSTRAP = os.environ.get("AQR_EMBARGO_IN_BOOTSTRAP", "1") == "1"   # also bootstrap-CI the embargoed-holdout premium (adds ~1 Ridge fit/draw)
...
def chrono_embargo_r2(X, y, dates, embargo):
    """Embargoed chronological holdout — the SAME test window as r2_off
    (test >= CHRONO_TEST_START) but with the `embargo` sampled trading days
    immediately before the boundary dropped from train. This is the
    apples-to-apples stricter honest baseline: identical test support to r2_off,
    so `r2_on - r2_off_embargo` isolates whether the single split's honest
    baseline was inflated by boundary/adjacency leakage (the JPX Target resolves
    ~T+2). One Ridge fit; returns r2 (NaN if the split degenerates).
    """
    date_vals = np.asarray(dates.values)
    test_start = np.datetime64(pd.Timestamp(CHRONO_TEST_START))
    uniq = np.unique(date_vals)
    ts_pos = int(np.searchsorted(uniq, test_start, side="left"))
    if ts_pos - embargo < 1:
        return float("nan")
    pos = np.searchsorted(uniq, date_vals)
    tr = pos < (ts_pos - embargo)            # train ends `embargo` sampled days before the boundary
    te = date_vals >= test_start             # identical test window to r2_off
    if int(tr.sum()) < 10 or int(te.sum()) < 10:
        return float("nan")
    reg = Ridge(alpha=RIDGE_ALPHA).fit(X[tr], y[tr])
    return float(r2_score(y[te], reg.predict(X[te])))


def walkforward_pooled_r2(X, y, dates, n_splits, embargo):
    """Expanding-window walk-forward R^2 over the whole sampled timeline, with an
    embargo gap — a multi-period robustness check, NOT a leakage premium (its
    test support differs from r2_on, so it is intentionally not differenced
    against r2_on).

    Splits on UNIQUE sampled trading dates (panel-safe: every row sharing a date
    lands on the same side). For each of n_splits contiguous test blocks, train
    on all rows strictly before it minus an `embargo` of sampled trading days,
    then POOL the out-of-fold predictions and score once (pooled R^2 avoids
    over-weighting small/noisy early folds). Numeric features are backward-
    looking, so only the label needs the gap. Returns (pooled R^2, n_folds).
    """
    date_vals = np.asarray(dates.values)
    uniq = np.unique(date_vals)
    n = len(uniq)
    ...
    y_true, y_pred, folds = [], [], 0
    for k in range(1, n_splits + 1):
        lo, hi = int(edges[k]), int(edges[k + 1])
        train_end = lo - embargo
        if train_end < 1 or hi <= lo:
            continue
        tr = pos < train_end
        te = (pos >= lo) & (pos < hi)
        if int(tr.sum()) < 10 or int(te.sum()) < 10:
        ...
        reg = Ridge(alpha=RIDGE_ALPHA).fit(X[tr], y[tr])
        y_true.append(y[te])
        y_pred.append(reg.predict(X[te]))
        folds += 1
    if folds == 0:
        return float("nan"), 0
    return float(r2_score(np.concatenate(y_true), np.concatenate(y_pred))), folds


def fit_and_score(X: np.ndarray, y: np.ndarray, dates: pd.Series, groups: pd.Series, with_embargo: bool = True, with_walkforward: bool = True) -> dict:
    ...
    r2_off_embargo = (
        chrono_embargo_r2(X, y, dates, EMBARGO_DAYS) if with_embargo else float("nan")
    )
    if with_walkforward:
        r2_off_wf, n_wf_folds = walkforward_pooled_r2(X, y, dates, WF_N_SPLITS, EMBARGO_DAYS)
    else:
        r2_off_wf, n_wf_folds = float("nan"), 0

    return {
        "r2_off": r2_off,
        "r2_on": r2_on,
        "r2_off_embargo": r2_off_embargo,
        "r2_off_wf": r2_off_wf,
...
def bootstrap_premium_ci(
    ...
    intact. NOTE: this respects WITHIN-stock correlation but not market-wide /
    date correlation — a date-block or two-way (stock x date) bootstrap would;
    that is left as a documented limitation, not folded into this CI. A stock
    ...
    reductions_emb, base_ps_emb, adapter_ps_emb = [], [], []
    for b in range(n_bootstrap):
        ...
        try:
            bs = fit_and_score(X_base[idx], y[idx], d, g, with_embargo=EMBARGO_IN_BOOTSTRAP, with_walkforward=False)
            asc = fit_and_score(X_adapter[idx], y[idx], d, g, with_embargo=EMBARGO_IN_BOOTSTRAP, with_walkforward=False)
        ...
        # Embargoed-holdout premium (r2_on - r2_off_embargo) per draw — the
        # apples-to-apples stricter premium (same 2021 test window as r2_off,
        # boundary purged). Walk-forward is NOT bootstrapped (different test
        # support; reported as a point robustness R^2 on full data only). Skip
        # the draw if either side is NaN so the three arrays stay row-aligned.
        if EMBARGO_IN_BOOTSTRAP and np.isfinite(bs["r2_off_embargo"]) and np.isfinite(asc["r2_off_embargo"]):
            bpe = bs["r2_on"] - bs["r2_off_embargo"]
            ape = asc["r2_on"] - asc["r2_off_embargo"]
            base_ps_emb.append(bpe)
            adapter_ps_emb.append(ape)
            reductions_emb.append(bpe - ape)
        if (b + 1) % 25 == 0:
            print(
    ...
    return {
        ...
        "premium_reduction_embargo": ci(reductions_emb),
        "base_premium_embargo": ci(base_ps_emb),
        "adapter_premium_embargo": ci(adapter_ps_emb),
        "n_bootstrap_embargo": len(reductions_emb),
    }
...
def main():
    ...
    def load_adapter():
        # FULL_WEIGHT_DIR override: load the merged CPT checkpoint as a
        # standalone causal-LM (no PEFT). Backwards compatible — without the
        # env var, the original LoRA path runs unchanged.
        if FULL_WEIGHT_DIR is not None:
            return AutoModelForCausalLM.from_pretrained(
                str(FULL_WEIGHT_DIR),
                torch_dtype=torch.bfloat16,
                trust_remote_code=True,
    ...
    if FULL_WEIGHT_DIR is not None:
        have_adapter = FULL_WEIGHT_DIR.exists() and any(FULL_WEIGHT_DIR.iterdir())
        adapter_source_label = f"full-weight checkpoint at {FULL_WEIGHT_DIR}"
    else:
        have_adapter = ADAPTER_DIR.exists() and any(ADAPTER_DIR.iterdir())
        adapter_source_label = f"LoRA adapter at {ADAPTER_DIR}"
    if not have_adapter:
        print(
            f"eval.py: WARNING — no model found ({adapter_source_label}); "
            f"evaluating BASE only (sanity-check mode)",
            flush=True,
        )
    else:
        print(f"eval.py: comparing BASE vs {adapter_source_label}", flush=True)
    ...
    if adapter_embs is not None:
        ...
        nan_ci = {"p2.5": float("nan"), "p50": float("nan"), "p97.5": float("nan"), "std": float("nan")}
        cis = {
            "premium_reduction": nan_ci,
            "base_premium": nan_ci,
            "adapter_premium": nan_ci,
            "n_bootstrap_completed": 0,
            "premium_reduction_embargo": nan_ci,
            "base_premium_embargo": nan_ci,
            "adapter_premium_embargo": nan_ci,
            "n_bootstrap_embargo": 0,
        }
    ...
    # Embargoed chronological holdout — SAME 2021 test window as r2_off with the
    # EMBARGO_DAYS sampled days before the boundary dropped from train.
    # premium_embargo = r2_on - r2_off_embargo is the apples-to-apples "does the
    # leakage premium survive purging the boundary" check (identical test support).
    _boe = base_scores.get("r2_off_embargo", float("nan"))
    _aoe = adapter_scores.get("r2_off_embargo", float("nan"))
    _bpe = base_scores["r2_on"] - _boe
    _ape = adapter_scores["r2_on"] - _aoe
    print(f"base_r2_off_embargo:   {_boe:.4f}", flush=True)
    print(f"r2_off_embargo:        {_aoe:.4f}", flush=True)
    print(f"base_leakage_premium_embargo: {_bpe:.4f}", flush=True)
    print(f"leakage_premium_embargo:      {_ape:.4f}", flush=True)
    print(f"premium_reduction_embargo:    {_bpe - _ape:.4f}", flush=True)
    print(
        f"premium_reduction_embargo_ci95: [{cis['premium_reduction_embargo']['p2.5']:.4f}, "
        f"{cis['premium_reduction_embargo']['p97.5']:.4f}] (median {cis['premium_reduction_embargo']['p50']:.4f})",
        flush=True,
    )
    print(
        f"base_premium_embargo_ci95:    [{cis['base_premium_embargo']['p2.5']:.4f}, "
        f"{cis['base_premium_embargo']['p97.5']:.4f}] (median {cis['base_premium_embargo']['p50']:.4f})",
        flush=True,
    )
    print(
        f"adapter_premium_embargo_ci95: [{cis['adapter_premium_embargo']['p2.5']:.4f}, "
        f"{cis['adapter_premium_embargo']['p97.5']:.4f}] (median {cis['adapter_premium_embargo']['p50']:.4f})",
        flush=True,
    )
    # Walk-forward multi-period OOS R^2 — robustness only (different test support
    # from r2_on, so intentionally NOT differenced into a premium).
    print(f"base_r2_off_walkforward: {base_scores.get('r2_off_wf', float('nan')):.4f}", flush=True)
    print(f"r2_off_walkforward:      {adapter_scores.get('r2_off_wf', float('nan')):.4f}", flush=True)
    print(f"n_walkforward_folds:     {base_scores.get('n_wf_folds', 0)}", flush=True)
    print(f"n_bootstrap_embargo:     {cis.get('n_bootstrap_embargo', 0)}", flush=True)
    _nbe = cis.get("n_bootstrap_embargo", 0)
    _nbc = cis.get("n_bootstrap_completed", 0)
    if _nbc and _nbe < 0.9 * _nbc:
        print(f"eval.py: WARNING embargo bootstrap kept {_nbe}/{_nbc} draws (<90%) — embargo CI may be biased", flush=True)
    ...
    print("eval_methodology:      GroupKFold(stock-disjoint)+clustered-bootstrap+embargo-holdout+walkforward", flush=True)
