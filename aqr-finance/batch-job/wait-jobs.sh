...
for slug in "${slugs[@]}"; do
  ...
  if [ "$state" = "succeeded" ]; then
    vesslctl job logs --limit 1000 "$slug" 2>&1 \
      | grep -E "^r2_leakage_off:|^r2_leakage_on:|^leakage_premium:|^base_r2_leakage_off:|^base_r2_leakage_on:|^base_leakage_premium:|^premium_reduction:|^r2_off_embargo:|^base_r2_off_embargo:|^leakage_premium_embargo:|^base_leakage_premium_embargo:|^premium_reduction_embargo:|^r2_off_walkforward:|^base_r2_off_walkforward:|^val_loss_final:|^val_loss_first30min:|^training_seconds:|^total_seconds:|^peak_vram_mb:|^num_trainable_M:|^num_trainable_pct:|^num_train_tokens_M:|^---" \
      | tail -40
  else
