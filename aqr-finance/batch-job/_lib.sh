...
# Dump the full job log. The vesslctl API caps --limit at 1000. An AQR
# training run is long (~23 h on a single H100), so the bulk of the log is
# loss-curve lines from train.py. Use \r-overwritten progress lines and a
# `^---` summary block at the end so a 1000-line dump always captures the
# summary regardless of run length.
dump_job_logs() {
