...
# while attention/MLP shift to the new domain). The 35 B base + this LoRA
# adapter fits comfortably on a single H100 80 GB, so this LoRA arm runs
# single-GPU by choice — no multi-GPU sharding needed. (Full-weight 35 B
# continued PT, which trains every parameter, does NOT fit one card; that is
# the separate 8×H100 FSDP2 arm under ../multigpu/.) Dry-run trained the full
# 945 M trainable footprint to 47 % (NaN-free, loss 2.31 → 2.20).
LORA_TARGET_MODULES = [
